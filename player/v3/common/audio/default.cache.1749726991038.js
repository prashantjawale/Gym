...
function setAudioModeOn() {
    if (document.getElementsByClassName('mgPlayerJSTest_play-step-audio-on') &&
        ...
        if (userPrefAudio) {
            ...
            let action = "mgPlayerJSTest_action:set_audio_storage";
            let data = {
...
GmCXt.playStepAudio = function(message) {
    ...
    if (!message || !message.data) {
        if (GmCXt.playerI) {
            let step = getStepFromPlayerI(GmCXt.playerI.currentStepId);
            message = {
    ...
    let play = function(url) {
        ...
        let action = "mgPlayerJSTest_action:start_step_completion_timeout";
        let data = {
            step: stepObj
        };
